  ...
  for (_, antennas) in map.iter() {
    ...
    for antenna_x in antennas {
      for antenna_y in antennas {
        ...
        let mut antinode_n = 0i64;

        loop {
          let antinode = (antenna_y.0 + diff_rows * antinode_n, antenna_y.1 + diff_cols * antinode_n);
          if antinode.0 < 0 || antinode.0 >= rows || antinode.1 < 0 || antinode.1 >= columns { break; }
